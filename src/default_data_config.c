...
static_assert((SPLINTERDB_MAX_VALUE_SIZE + sizeof(message_encoding)
               == MAX_MESSAGE_SIZE),
              "Message encoding header size mismatch");
               ...
               size_t      *out_encoded_len)
{
   ...
   if (value_len + sizeof(message_encoding) > dst_msg_buffer_len) {
      platform_error_log(
         "splinterdb.encode_value: "
         "value_len %lu + encoding header %lu exceeds buffer size %lu",
         value_len,
...
static int
decode_message_value(size_t       msg_buffer_len,
                     const void  *msg_buffer,
                     const char **out_value,
                     size_t      *out_value_len)
{
   if (msg_buffer_len < sizeof(message_encoding)) {
      platform_error_log("splinterdb.decode_message_value: message too short");
      return EINVAL;
   }
...
)
{
   ...
   platform_assert(max_value_size <= SPLINTERDB_MAX_VALUE_SIZE);
   ...
   data_config cfg = {
      ...
      .decode_message_value = decode_message_value,
      .context              = NULL,
   };
